...
Для запуска используй `/plan`, `/sdd-new`, `/review`, `/evidence`.

> Справочник по командам, хукам и end-to-end workflow: [`.gigacode/docs/COMMANDS-AND-WORKFLOW.md`](docs/COMMANDS-AND-WORKFLOW.md).
...
Schema: `schemas/evidence-bundle.schema.json`. Используй `/evidence` для генерации.
...
- ❌ Не модифицировать тесты, чтобы пройти их (это меняет acceptance criteria). Enforced хуком `test-files-protector.sh`: правки в `test/*` блокируются без `PDLC_ALLOW_TEST_EDIT=1` — выставляет только Test Agent.
