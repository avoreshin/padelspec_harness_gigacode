...
**Test Agent — единственная роль, которой разрешено** редактировать тесты. Перед началом работы открой окно правки:

```bash
bash .gigacode/scripts/test-edit-window.sh open "фаза Test, задача <slug>"
```

После того как failing-тесты написаны и прогнаны — **обязательно** закрой его:

```bash
bash .gigacode/scripts/test-edit-window.sh close
```

Окно нужно **только** на время этой подзадачи. Оставленное открытым, оно снимает гейт и для Coding Agent в той же сессии.

> Раньше здесь стоял `export PDLC_ALLOW_TEST_EDIT=1`. Так это не работает: каждый вызов Bash-инструмента — отдельный процесс, а хук запускается рантаймом из своего, переменная между ними не передаётся. Разрешение передаётся файлом-маркером, который видят оба. Переменная осталась рабочей только там, где окружение процесса задаёт оператор — то есть в CI.
