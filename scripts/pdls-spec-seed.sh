...
#   discover <path>          декомпозировать репозиторий на capability по package
...
#   manifest                 индекс .kb/specs/*.md (таблица + Dependencies) → manifest.md
#
# Exit-codes:
#   0 — ok
...
cmd_validate() {
  ...
  local out
  if out="$(bash .gigacode/scripts/validate-schemas.sh validate capability-spec "$file" 2>&1)"; then
