#!/usr/bin/env bash
# pdls-spec-seed.sh — детерминированные guardrails для /pdls-spec-seed.
#
...
# .gigacode/commands/pdls-spec-seed.md. Этот скрипт — детерминированная опора
...
cmd_discover() {
  ...
  local files
  files="$(find "$path" -type f \( -name '*.java' -o -name '*.kt' \) 2>/dev/null | sort)"
...
# Печатает в stdout И пишет <parent>/manifest.md. Без LLM.
cmd_manifest() {
  local dir="${1:-$KB_DIR}"
  ...
  local out; out="$(dirname "$dir")/manifest.md"
