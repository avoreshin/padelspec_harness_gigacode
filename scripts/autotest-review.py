...
def review_case(case, scenario):
    ...
    # Каналы, названные именно в шагах-проверках. Считать канал по всему
    # сценарию нельзя: «отправить сообщение в топик X» — шаг подготовки, и он
    # один вносит kafka в общий набор. Сценарий, который шлёт в Kafka, а
    # проверяет лог на машине, — ровно тот случай, ради которого написана вся
    # сверка, — при подсчёте по всему телу выглядел бы чистым.
    assert_channels = set()
    for s in steps:
        if is_assertion(s):
            assert_channels.update(channels_of(s))

    # 1. Канал проверки. Главная находка: проверка ушла не туда.
    # Шаги-проверки канал не назвали вовсе — падаем на весь сценарий: тогда
    # утверждать «проверка ушла в другой канал» не на чем, и лишний blocker
    # на нейтральной формулировке хуже пропуска. Сценарий вообще без проверок
    # ловится отдельно — правилом 3.
    for n, st in enumerate(model_steps(case), start=1):
        expected = st.get("expected_result") or ""
        want = primary_channel(expected)
        if not want:
            continue
        where = assert_channels or actual_channels
        if want in where:
            continue
        got = sorted({c for c in where if c != want})
