...
#
# ВЫРАВНИВАНИЕ ЗДЕСЬ ЗНАЧИМО. scripts/build-gigacode-extension.sh подменяет эти
# строки точным совпадением, чтобы в расширении путь шёл через $HARNESS_ROOT.
# Промах патча тихий: общая замена подставит переменную внутрь одинарных кавычек,
# она не раскроется, и валидация markdown в расширении отвалится с «extractor
# missing». Меняешь строку — правь и патч в сборщике.
extractor_for_schema() {
  case "$1" in
    ...
    capability-spec) printf '%s\n' "$HARNESS_ROOT/scripts/extract-capability-frontmatter.sh" ;;
    *)              printf '\n' ;;
  esac
}
