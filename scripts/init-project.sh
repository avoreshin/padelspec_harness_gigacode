...
# VERSION в целевой репозиторий не копируется (он про версию проекта, а не
# обвязки), поэтому session-status.sh не смог бы назвать версию установленной
# обвязки. Кладём её отдельным маркером внутрь .gigacode/.
if [[ -f "$SRC/VERSION" ]]; then
  tr -d '[:space:]' < "$SRC/VERSION" > "$TARGET/.gigacode/.harness-version"
fi
