...
def sdd_block(project, cases, sdd_dir="docs/sdd"):
    ...
    # Единица покрытия — SDD, а не пара (story, AC): один документ может
    # закрывать группу story, и его критерии не обязаны дублироваться по каждой.
    by_sdd = {}
    for story in sorted(covering):
        by_sdd.setdefault(coverage._key(covering[story]), covering[story])
    acs = [(key, ac) for key, sdd in by_sdd.items() for ac in sdd["_ac"]]
    block = {
        ...
        "documents": [
            {"sdd": key,
             "stories": [s for s in sorted(covering)
                         if coverage._key(covering[s]) == key],
             "path": sdd["_path"],
