...
# py — python3 в UTF-8 Mode (PEP 540).
# Без него на Windows с не-UTF-8 локалью (cp1251/cp866) Python берёт кодировку из
# локали и для stdout, и для open(): `hooks list` падает с UnicodeEncodeError на
# эмодзи, а чтение settings.json даёт mojibake. Файлы харнесса всегда UTF-8,
# поэтому кодировку фиксируем явно, а не полагаемся на локаль хоста.
...
# jq — required, а не optional: на нём держится разбор события у 12 из 16 хуков.
# Без него enforcement-слой отказывает целиком (fail-closed, см. hooks/lib/require-jq.sh),
# поэтому доктор обязан краснеть, а не рапортовать «всё хорошо».
REQ_BINS=("git" "jq" "node" "npm")
...
hint_for() {
  case "$1" in
    ...
    jq)         echo "brew install jq  |  apt-get install jq  |  winget install jqlang.jq" ;;
...
cmd_hooks() {
  ...
  case "$sub" in
    ...
    *)       err "unknown: hooks $sub (list|status|enable|disable)"; exit 2 ;;
  esac
}

# hooks status — машиночитаемый TSV для чекбокс-UI команды /harness.
# Одна строка на уникальный хук-скрипт:  STATE<TAB>NAME<TAB>EVENTS
#   STATE  = enabled | disabled
#   NAME   = имя .sh
#   EVENTS = события через запятую (где хук привязан)
hooks_status() {
  py - "$SETTINGS" <<'PY'
import json,sys,re,collections
d=json.load(open(sys.argv[1],encoding='utf-8'))
def name(c):
    m=re.search(r'([\w-]+\.sh)',c); return m.group(1) if m else (c or '')[:48]
enabled=collections.OrderedDict()
for ev,arr in d.get('hooks',{}).items():
    for grp in arr:
        for hk in grp.get('hooks',[]):
            enabled.setdefault(name(hk.get('command','')),set()).add(ev)
disabled=collections.OrderedDict()
for r in d.get('$disabled_hooks',[]):
    disabled.setdefault(name(r['hook'].get('command','')),set()).add(r.get('event','?'))
for nm,evs in enabled.items():
    print(f"enabled\t{nm}\t{','.join(sorted(evs))}")
for nm,evs in disabled.items():
    if nm not in enabled:
        print(f"disabled\t{nm}\t{','.join(sorted(evs))}")
PY
}
...
hooks_toggle() {
  ...
  local result
  result="$(py - "$SETTINGS" "$action" "$nm" <<'PY'
import json,sys,re
...
# ---------------------------------------------------------------------------
# test — прогнать hook-doctor по установленному расширению (или по .gigacode/)
# ---------------------------------------------------------------------------

VERIFY_HOOKS=".gigacode/scripts/verify-gigacode-hooks.sh"

cmd_test() {
  [ -f "$VERIFY_HOOKS" ] || { err "$VERIFY_HOOKS не найден"; exit 2; }
  bash "$VERIFY_HOOKS" "$@"
}
...
${C_BOLD}harness.sh${C_RESET} — control panel обвязки
  ...
  ${C_CYAN}hooks list${C_RESET}              показать активные/отключённые хук-скрипты (для человека)
  ${C_CYAN}hooks status${C_RESET}            TSV state/name/events (для чекбокс-UI /harness)
  ...
  ${C_CYAN}test${C_RESET} [EXT_DIR]          hook-doctor: проверить хуки установленного расширения
...
Примеры:
  ...
  bash .gigacode/scripts/harness.sh test ~/.gigacode/extensions/padelspec-harness
EOF
  exit 2
}
