...
def resolve_sdd_dir(path, data):
    """Каталог SDD по профилю. Как и project_root, путь записан относительно
    самого профиля: профиль переезжает вместе с репозиторием, а CWD у
    следующего запуска будет другим. Пусто — решает вызывающий."""
    sdd = str(data.get("sdd_dir") or "").strip()
    dev = str(data.get("dev_repo") or "").strip()
    if not sdd and not dev:
        return ""
    if not sdd:
        sdd = os.path.join(dev, "docs", "sdd")
    elif dev and not os.path.isabs(sdd):
        sdd = os.path.join(dev, sdd)
    return resolve_root(path, sdd)


def sdd_dir_for(project, out=None):
    """→ каталог SDD для проекта либо "" — точка входа для соседних скриптов."""
    ...
    try:
        ...
        return ""
    return resolve_sdd_dir(path, data)
...
# Поля, которые автоматика не выводит: их ставит человек, и `refresh` обязан их
# сохранить. Frontmatter при обновлении собирается заново, поэтому всё, что не
# перечислено здесь, потерялось бы молча.
HUMAN_FIELDS = ("dev_repo", "sdd_dir")


def render(project_root, facts, note="", human=None):
    ...
    lines = [
        ...
        # Репозиторий разработки и каталог SDD в нём. Пусто — значит SDD лежат
        # рядом с тест-моделью; так бывает в самодостаточном проекте, но в
        # типовом раскладе автотесты живут отдельно от разработки, и без этих
        # двух строк сверка с acceptance criteria искала бы SDD не там.
        "dev_repo: %s" % (human.get("dev_repo") or ""),
        "sdd_dir: %s" % (human.get("sdd_dir") or ""),
        ...
        "",
        "```",
        "/pdls:test-project refresh        # дельта фактов и описания",
        "```",
        "",
    ]
    if note:
...
def cmd_init(args):
    ...
    if not root:
        ...
        root = root.replace("\\", "/")
    human = {"dev_repo": args.dev_repo, "sdd_dir": args.sdd_dir}
    io.open(path, "w", encoding="utf-8").write(render(root, facts, note, human))
...
def cmd_check(args):
    ...
    print("SDD:          %s" % (resolve_sdd_dir(path, data) or "рядом с тест-моделью (docs/sdd)"))
    ...
    print("")
    print("Актуализировать: /pdls:test-project refresh")
    return 1
...
def cmd_refresh(args):
    ...
    human = {k: data.get(k) for k in HUMAN_FIELDS}
    head = render(root, facts, "", human).split("\n---\n", 1)[0]
...
def main():
    ...
    ap.add_argument("--dev-repo", default="", help="корень репозитория разработки (для init)")
    ap.add_argument("--sdd-dir", default="", help="каталог SDD; путь относительно профиля либо абсолютный")
