...
#
# Извлекает:
#   - Metadata-таблицу (id, author, status, risk_class, created, story, ...)
#   - Goal text
...
STORY_RAW="$(strip_md "$(get_meta Story)")"
...
# Story — связь SDD с задачей трекера. Поле опционально: SDD, написанные до его
# появления, обязаны остаться валидными. Значений может быть несколько (один SDD
# на группу story) — разделители запятая и пробел, шаблонная заглушка
# `<ключ(и) ...>` в результат не идёт.
STORIES_ARRAY="$(printf '%s' "$STORY_RAW" \
