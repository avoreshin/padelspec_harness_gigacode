...
for path_pattern in "${PROTECTED_PATHS[@]}"; do
  if printf '%s' "$FILE_PATH" | grep -qE -- "$path_pattern"; then
    deny "Writing to protected path: $FILE_PATH. Secrets must not be committed to repo. Instead: keep the value in an environment variable or a git-ignored local file and reference it by name."
  fi
done
...
for pattern in "${SECRET_PATTERNS[@]}"; do
  if printf '%s' "$CONTENT" | grep -qE -- "$pattern"; then
    deny "Detected secret-like pattern in content ($FILE_PATH). Do not commit secrets. Instead: replace the literal with an env-var reference or a placeholder; for fixtures use an obviously fake value."
  fi
done
