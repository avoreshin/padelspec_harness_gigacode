...
if [[ "$CURRENT_TOKENS" -ge "$BUDGET" ]]; then
  cat >&2 <<EOF
{"decision": "block", "reason": "Cost circuit breaker tripped. Risk class $RISK_CLASS, budget $BUDGET tokens, consumed $CURRENT_TOKENS. Either: (1) finalize current task and /evidence, (2) escalate risk class with justification, (3) request budget increase via human review. §4.4 PDLC v3.5."}
EOF
  exit 2
fi
