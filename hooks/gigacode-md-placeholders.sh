...
#   - <номер>, <текст> in the §10 read-prefix notation — это обозначение формата
#     вывода инструмента, а не незаполненное поле конфигурации
PLACEHOLDERS=$(awk '
  /^```/ { in_code = !in_code; next }
  in_code { next }
  /<type>|<scope>|<subject>|<feature>|<N>|<NNNN>|<slug>|<имя>|<номер>|<текст>|<SME, security, architect>/ { next }
  match($0, /<[^>]+>/) {
    line_no = NR
