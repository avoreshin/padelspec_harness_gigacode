...
# Записать факт отказа в append-only audit trail.
#
# ЗАЧЕМ. `jsonl-audit-sink` висит на PostToolUse, а заблокированный вызов до
# PostToolUse не доходит — инструмент не запускается вовсе. Получалось, что в
# трейл попадало всё, кроме единственных событий, ради которых policy-слой и
# существует: сработавших гейтов. §2.2 P6 (append-only durable state) требует
# обратного — фиксируется прежде всего то, что изменило исход.
#
# Пишется здесь, а не отдельным хуком: через deny() проходит каждая блокировка
# всех хуков обвязки, и это единственная точка, где отказ гарантированно виден.
# Любая ошибка записи проглатывается: аудит не имеет права помешать блокировке.
_hook_audit_deny() {
  local reason="$1" event="$2"
  local root; root="$(harness_root 2>/dev/null)" || return 0
  [[ -n "$root" && -d "$root" ]] || return 0
  local dir="$root/.gigacode/audit"
  mkdir -p "$dir" 2>/dev/null || return 0
  local ts; ts="$(date -u +"%Y-%m-%dT%H:%M:%S.%3NZ" 2>/dev/null || date -u +"%Y-%m-%dT%H:%M:%SZ")"
  local risk="R1"
  if [[ -f "$root/.gigacode/.cost/current-risk-class" ]]; then
    risk="$(tr -d '[:space:]' < "$root/.gigacode/.cost/current-risk-class" 2>/dev/null || echo R1)"
    [[ "$risk" =~ ^R[0-5]$ ]] || risk="R1"
  fi
  # Имя хука — первый кадр стека, который не сам common.sh: половина хуков зовёт
  # block(), и это ещё один кадр внутри библиотеки. Индекс наугад давал бы в
  # трейле «common» вместо имени сработавшего гейта.
  local hook="unknown" frame
  for frame in "${BASH_SOURCE[@]:1}"; do
    case "$frame" in
      */common.sh|common.sh) continue ;;
      "") continue ;;
      *) hook="$(basename -- "$frame" .sh)"; break ;;
    esac
  done
  local tool; tool="$(hook_field_any '.tool_name' '.tool' 2>/dev/null || true)"
  local session; session="$(hook_field '.session_id' 2>/dev/null || true)"
  jq -nc --arg ts "$ts" --arg ev "$event" --arg tl "$tool" --arg rc "$risk"          --arg hk "$hook" --arg rs "${reason:0:500}" --arg sid "$session" '{
    timestamp: $ts,
    event:     $ev,
    agent:     "main",
    tool:      $tl,
    ...
    >> "$dir/$(date -u +%Y-%m-%d).jsonl" 2>/dev/null || true
  return 0
}
...
deny() {
  ...
  _hook_audit_deny "$reason" "$event" || true
