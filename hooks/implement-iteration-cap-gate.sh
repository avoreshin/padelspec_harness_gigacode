...
# emit-phase-event.sh принимает необязательный `--loop <dev|testing>` ПЕРВЫМ
# аргументом. Без его среза позиционные съезжают на два: `--loop dev <task>
# test implement iterate 4` давало TO=test, и запись четвёртой итерации
# проходила мимо кэпа. Флаг ввёл цикл тестирования, а снимало это гейт
# dev-цикла — тот же класс, что `--loop testing` внутри reason в v0.12.4.
if [ "${1:-}" = "--loop" ] && [ "$#" -ge 2 ]; then
  shift 2
fi
