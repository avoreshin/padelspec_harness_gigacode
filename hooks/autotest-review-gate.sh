...
if printf '%s' "$COMMAND" | grep -q 'export-testcases-xml\.py'; then
  ...
  MODE="close"
  # `close --force <причина>` — осознанный выход из несошедшегося цикла. Без
  # него BLOCK оставляет окно правки тестов открытым до конца сессии, то есть
  # test-files-protector молча выключенным: хуже, чем закрытие с записью в
  # журнал окна. Гейт при этом не молчит — печатает находки в stderr.
  # Не `grep -q … && MODE=…`: под `set -e` из common.sh безрезультатный grep
  # сделал бы весь список неуспешным и убил хук с кодом 1 — для рантайма это
  # «хук не сработал», то есть тихий пропуск политики.
  if printf '%s' "$COMMAND" | grep -qE '[[:space:]]--force([[:space:]]|$)'; then
    MODE="close-force"
  fi
...
if [ "$MODE" = "close-force" ]; then
  printf '%s\n' "[autotest-review-gate] окно закрывается ПРИНУДИТЕЛЬНО при вердикте BLOCK ($BLOCKED)." >&2
  printf '%s\n' "Находки не сняты и остаются в силе:" >&2
  printf '%s\n' "$SUMMARY" >&2
  printf '%s\n' "Свод по релизу обязан их назвать: python3 .gigacode/scripts/collect-test-evidence.py $BLOCKED" >&2
  exit 0
fi
