...
if [[ "$RISK_CLASS" =~ ^R[3-5]$ ]] && [[ "$ACTIVE_SDD" == "none" ]]; then
  deny "Сжатие контекста при risk class $RISK_CLASS, но в docs/sdd/ нет активного SDD. Для R3+ SDD обязателен. Заведи спецификацию через /sdd-new либо понизь класс риска с обоснованием — и повтори сжатие. Snapshot сохранён в $SNAPSHOT_FILE. §4.5 PDLC v3.5."
fi
