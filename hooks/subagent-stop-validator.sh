...
case "$AGENT_NAME" in
  ...
  test)
    ...
    if ! printf '%s' "$OUTPUT" | grep -qiE "(failed|passed)"; then
      deny "Вывод test-агента не содержит результата прогона (нет 'failed'/'passed'). TDD red требует доказательства, что тесты действительно запускались."
    fi
    ;;
  coding)
    ...
    if printf '%s' "$OUTPUT" | grep -qE "(tests?/|_test\.|\.spec\.)"; then
      deny "Coding-агент, судя по отчёту, трогал тестовые файлы. Это нарушает §7 GIGACODE.md: тесты — acceptance criteria, менять их может только test-агент."
    fi
    ;;
  review)
    ...
    if ! printf '%s' "$VERDICT_LINE" | grep -qE "(APPROVE|REQUEST_CHANGES|BLOCK)"; then
      deny "Строка verdict у review-агента должна быть одной из APPROVE / REQUEST_CHANGES / BLOCK. Получено: $VERDICT_LINE"
    fi
    ;;
  security)
    ...
    if ! printf '%s' "$VERDICT_LINE" | grep -qE "(PASS|FAIL|NEEDS_HUMAN_REVIEW)"; then
      deny "Строка verdict у security-агента должна быть одной из PASS / FAIL / NEEDS_HUMAN_REVIEW. Получено: $VERDICT_LINE"
    fi
    ;;
esac
