...
# === STEP 0b: сессия ведёт цикл тестирования — доказательство собирает он ===
# Цикл тестирования всегда оставляет дифф: сценарии в feature-файлах, правку
# тест-модели, свод по релизу. Проверки STEP 0 этого не различают, и в
# репозитории автотестов — где разработку не ведут вовсе — сессия упиралась в
# требование dev-артефакта, который живёт в другом репозитории. Закрытие там
# держит test-loop-enforcer, и держит своим сводом.
#
# Различаем по последнему фазовому переходу в трейле, а не по наличию testing-
# событий вообще: репозиторий, где однажды прогнали тестовый цикл, обязан
# вернуться под этот гейт, как только в нём снова пойдёт разработка.
AUDIT_DIR=".gigacode/audit"
if [[ -d "$AUDIT_DIR" ]]; then
  LAST_PHASE="$(grep -h '"event":"phase_transition"' "$AUDIT_DIR"/*.jsonl 2>/dev/null | tail -1 || true)"
  if [[ -n "$LAST_PHASE" && "$LAST_PHASE" == *'"loop":"testing"'* ]]; then
    echo "[evidence-bundle-enforcer] последний переход — цикл тестирования; закрытие держит test-loop-enforcer" >&2
    exit 0
  fi
fi
...
if [[ ! -f "$SCHEMA_PATH" ]]; then
  emit_block "Schema not found at $SCHEMA_PATH. Cannot validate evidence bundle without source of truth. Restore the schema from the harness (.gigacode/schemas/) or point EVIDENCE_SCHEMA_PATH at it."
fi

# === STEP 2: bundle — валидный JSON ===
if ! jq empty "$BUNDLE_PATH" 2>/dev/null; then
  emit_block "Evidence bundle is not valid JSON: $BUNDLE_PATH. Regenerate it with /evidence instead of hand-editing; a trailing comma is the usual cause."
fi

# === STEP 3: extract risk_class для условной логики ===
RISK_CLASS=$(jq -r '.risk_class // "unknown"' "$BUNDLE_PATH")
if ! [[ "$RISK_CLASS" =~ ^R[0-5]$ ]]; then
  emit_block "Evidence bundle has invalid or missing risk_class: '$RISK_CLASS'. Must be R0..R5. Set it with /risk-classify, then rerun /evidence."
fi
...
if command -v node >/dev/null 2>&1 && [[ -f "$VALIDATOR_JS" ]]; then
  # Код возврата снимается через `|| VALIDATOR_RC=$?`, а не следующей строкой:
  # хук наследует `set -e` из lib/common.sh, а присваивание из подстановки команд
  # возвращает код самой команды. При rc=1 (bundle не сошёлся со схемой) скрипт
  # умирал прямо здесь с exit 1 — а блокировкой рантайм считает только exit 2,
  # то есть ветка `1) emit_block` ниже была недостижима и сессия закрывалась с
  # невалидным bundle. Ровно тот отказ, который этот хук и должен ловить.
  VALIDATOR_RC=0
  VALIDATION_OUTPUT="$(node "$VALIDATOR_JS" validate "$SCHEMA_PATH" "$BUNDLE_PATH" 2>&1)" || VALIDATOR_RC=$?
  case "$VALIDATOR_RC" in
    0) VALIDATED=1 ;;
    1) emit_block "Evidence bundle failed schema validation. $VALIDATION_OUTPUT Fix the fields the validator names, then rerun /evidence." ;;
    *)
...
PY
      ...
      emit_block "Evidence bundle failed schema validation (python jsonschema). Errors: $VALIDATION_OUTPUT Fix the fields the validator names, then rerun /evidence."
    fi
  fi
fi
...
if [[ "$VALIDATED" -eq 0 ]]; then
  ...
  for field in schema_version task_id risk_class sdd_reference diff_summary tests completed_at; do
    if ! jq -e ".$field" "$BUNDLE_PATH" >/dev/null 2>&1; then
      emit_block "Evidence bundle missing required field: $field. Add it and rerun /evidence (smoke validation only; install ajv for the full schema check)."
    fi
  done
fi
...
if [[ "$TESTS_STATUS" != "passed" ]]; then
  emit_block "Evidence bundle reports tests.status=$TESTS_STATUS. Tasks cannot complete with non-passing tests. Fix the failing tests and rerun /evidence — do not edit the tests to make them pass."
fi

# 5a — sdd_reference must point to a real file (or be a conversation:// stub for R0)
SDD_REF=$(jq -r '.sdd_reference // ""' "$BUNDLE_PATH")
if [[ -z "$SDD_REF" ]]; then
  emit_block "Evidence bundle missing sdd_reference. Point it at the spec in docs/sdd/ that this task implements; create one with /sdd-new if there is none."
elif [[ "$SDD_REF" =~ ^conversation:// ]]; then
  if [[ "$RISK_CLASS" != "R0" ]]; then
    emit_block "sdd_reference=conversation://... is only valid for R0. Got risk_class=$RISK_CLASS. Write the spec to docs/sdd/ and reference that file instead."
  fi
elif [[ ! -f "$SDD_REF" ]]; then
  emit_block "sdd_reference points to '$SDD_REF' but that file does not exist. Fix the path or create the spec with /sdd-new."
fi
...
if command -v git >/dev/null 2>&1 && git rev-parse --is-inside-work-tree >/dev/null 2>&1; then
  ...
  if [[ "$ACTUAL" != "$CLAIMED" ]]; then
    emit_block "diff_summary.files_changed=$CLAIMED does not match git ($ACTUAL actual files, excluding bundle artefacts). Bundle is lying about scope. Rerun /evidence so the summary is taken from git rather than written by hand."
  fi
fi
...
case "$RISK_CLASS" in
  R3|R4|R5)
    ...
    if [[ "$SECURITY_VERDICT" != "PASS" ]]; then
      emit_block "Risk class $RISK_CLASS requires security_verdict.verdict=PASS. Got: $SECURITY_VERDICT. Run the security subagent and resolve its findings; a non-PASS verdict cannot be overwritten by hand. §4.5 PDLC v3.5."
    fi
    ;;
esac
...
case "$RISK_CLASS" in
  R4|R5)
    ...
    if [[ -z "$APPROVER" ]]; then
      emit_block "Risk class $RISK_CLASS requires human_signoff.approver. Ask a human to review and record their identity in the bundle — this field cannot be self-filled by the agent. §4.5 (separation of duties)."
    fi
    ...
    if [[ -z "$AUTHOR" || -z "$REVIEWER" || -z "$DEPLOYER" ]]; then
      emit_block "R$RISK_CLASS requires separation_of_duties_proof with author/reviewer/deployer all set. Collect the three names from the people who actually did each role and rerun /evidence."
    fi
    if [[ "$AUTHOR" == "$REVIEWER" || "$REVIEWER" == "$DEPLOYER" || "$AUTHOR" == "$DEPLOYER" ]]; then
      emit_block "R$RISK_CLASS separation of duties violated: author/reviewer/deployer must be three distinct people. Got: author=$AUTHOR reviewer=$REVIEWER deployer=$DEPLOYER. Involve a third person for the duplicated role; the rule cannot be satisfied by renaming."
    fi
    ;;
esac
...
if [[ "$RISK_CLASS" == "R5" ]]; then
  ...
  if [[ "$CAB_DECISION" != "approved" && "$CAB_DECISION" != "approved_with_conditions" ]]; then
    emit_block "R5 requires cab_decision.decision in [approved, approved_with_conditions]. Got: $CAB_DECISION. Take the change to the advisory board and record their decision; R5 has no agent-side path around this."
  fi
  REG_STATUS=$(jq -r '.regulatory_eval_results.status // "MISSING"' "$BUNDLE_PATH")
  if [[ "$REG_STATUS" != "passed" ]]; then
    emit_block "R5 requires regulatory_eval_results.status=passed. Got: $REG_STATUS. Rerun the regulatory eval suite and fix what it reports before finishing."
  fi
fi
