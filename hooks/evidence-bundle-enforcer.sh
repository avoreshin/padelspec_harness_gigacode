...
# 4a — ajv (preferred)
# Оба пакета обязаны быть в npx-окружении: ajv-cli@5 не тянет ajv-formats
# в зависимостях, а `-c ajv-formats` требует его require'нуть. Без -p
# ajv-formats любой вызов падает с module-not-found — и bundle блокируется
# как «невалидный», хотя схему он проходит. Ср. .gigacode/scripts/_validator.js,
# который ставит ajv@8 + ajv-formats@3 явно.
if command -v npx >/dev/null 2>&1; then
  if VALIDATION_OUTPUT=$(npx --yes -p ajv-cli -p ajv-formats ajv validate \
        -c ajv-formats \
        --spec=draft2020 \
        -s "$SCHEMA_PATH" \
