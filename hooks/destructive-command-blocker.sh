...
for seg in ${SEGMENTS[@]+"${SEGMENTS[@]}"} "$CMD_NORM"; do
  if is_dangerous_rm "$seg"; then
    deny "Destructive command blocked by BASE policy for risk class $RISK_CLASS. Рекурсивное принудительное удаление корня/домашнего каталога/маски: '$seg'. Command: '$COMMAND'. §4.4 PDLC v3.5."
  fi
done
...
if [[ "$RISK_CLASS" == "R5" ]]; then
  for seg in ${SEGMENTS[@]+"${SEGMENTS[@]}"}; do
    ...
    if [[ "$seg_allowed" -eq 0 ]]; then
      deny "Risk class R5 (regulated risk logic) requires human-driven Bash. Segment '$seg' is not in the read-only allowlist (command: '$COMMAND'). §4.5 PDLC v3.5: R5 autonomy = change_advisory_board, no autonomous Bash."
    fi
  done
fi
