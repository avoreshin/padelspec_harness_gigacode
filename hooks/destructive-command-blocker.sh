...
is_dangerous_rm() {
  ...
  # Цель: / , ~ , * , $HOME , ${HOME} — то есть смёт всего, а не конкретного
  # каталога. Хвостовые '/' и '/*' входят в цель: 'rm -rf ~/' и 'rm -rf ~' — это
  # одно и то же действие, а прежний шаблон требовал, чтобы цель кончалась
  # пробелом или концом строки, и косая черта его обходила. Кавычки снимаются до
  # сопоставления по той же причине: '"$HOME"' проходил мимо шаблона '$HOME'.
  local target
  target="$(printf '%s' "$seg" | tr -d "\"'")"
  printf '%s' "$target" | grep -qE ' (/|~|\*|\$HOME|\$\{HOME\})/?\*?( |$)' && return 0
  return 1
}
