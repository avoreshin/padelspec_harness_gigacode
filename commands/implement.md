...
1. **Identify SDD + check prior phase.** Если `{{args}}` пустой — попроси slug. Найди SDD в `docs/sdd/`. Проверь:
   ...
   - Если тестов нет — STOP, предложи `/test <slug>` сначала.

2. **Check iteration counter.** Выполни `bash .gigacode/scripts/workflow-state.sh <task-id>` (парсит `phase_transition` события из audit-trail). Iteration:
   - Если `status=not_found`/`no_audit` или `current_phase≠implement` → iteration=1
   - Если `current_phase=implement` и `last_status=iterate` → iteration = `iteration` из вывода + 1
   - Если iteration ≥ 4 — **STOP, mandatory escalation**, см. §10 ниже.

3. **For R2+: enforce worktree boundary.** Прочитай SDD risk_class:
   - R0/R1: можно работать на main checkout
   - R2+: проверь, что находишься в `.worktrees/<task-id>/`. Если нет — запусти `bash .gigacode/scripts/agentic-worktree.sh start <task-id>` и переключись туда.
...
5. **Validate structured output:** найди `<!-- phase-transition:json -->` блок. Validate через `bash .gigacode/scripts/check-phase-transition.sh`. Прочитай:
   - status: pass / iterate / escalate
   - next_phase
   - iteration (должен совпадать с подсчитанным в шаге 2)

6. **Record the transition (ОБЯЗАТЕЛЬНО).** Запиши phase_transition в audit-trail — без этого `/continue`, `/squash` и iteration cap не увидят фазу:

   ```bash
   bash .gigacode/scripts/emit-phase-event.sh <task-id> <from> implement <status> <iteration> "<коротко: причина/результат>"
   ```
...
8. **Next step (status=pass):**
   ...
   - `header`: "Impl→?"
   - `options`:
     - `label`: "Continue → /review (Recommended)" · `description`: "Передать на code review против SDD."
     - `label`: "Iterate (более clean реализация)" · `description`: "Hardening / refactor; тесты остаются pass."
     - `label`: "Stop & save" · `description`: "Сохранить state, выйти. /continue вернёт."

   **Fallback** (vendor без picker'а): напечатай numbered options и жди INPUT.

   В auto-mode (R0/R1) переход к `/review` автоматический.
...
9. **Retry (status=iterate):**
   ...
   - `header`: "Iterate"
   - `options`:
     - `label`: "Continue → /implement (Recommended)" · `description`: "Ещё одна итерация (n+1)."
     - `label`: "Revise SDD" · `description`: "Откат на /sdd-new — вероятен AC gap."
     - `label`: "Stop" · `description`: "Прервать loop, разобраться вручную."

   Если iteration = 3:
   ```
   ⚠ ПОСЛЕДНЯЯ итерация (3/3). Следующий fail = mandatory escalation на /sdd-new.
   ...
   - `options`:
     ...
     - `label`: "Revise SDD now" · `description`: "Откат на /sdd-new сейчас — если AC gap уже очевиден."
...
10. **Mandatory escalation (status=escalate, iteration ≥ 3):**
   ...
   Recovery rule (§7 phase-gate-protocol): mandatory rollback на /sdd-new для SDD revision.
   ...
   - `question`: "Iteration cap hit. Подтверждаешь rollback на /sdd-new?"
   - `header`: "Escalate"
   - `options`:
     - `label`: "Rollback → /sdd-new (Recommended)" · `description`: "Revise SDD с описанием AC gap (обязательно human review)."
...
## Правила (cross-ref агент'а `.gigacode/agents/coding.md`)

- Atomic commits (один логический шаг = один commit)
- **Запрещено** «подкрутить» тест — если test agent написал неправильный test, откатываемся на `/test` фазу
...
**Tip (resume):** если прервался посреди iter — `/continue <slug>` восстановит state (включая iteration counter) из audit-trail.
