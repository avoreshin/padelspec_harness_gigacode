...
1. **Validate context:**
   ...
   - Если `current_phase != evidence|done` → STOP, предложи `/continue <task-id>` сначала.
...
- ❌ Squash без проверки phase=evidence|done — STOP, рекомендуй /continue.
- ❌ Auto-push без user confirmation — INV-4.
- ❌ Squash 1 commit (нет смысла) — info-сообщение «уже один commit, squash не нужен».

## Edge cases

- **Merge commits in branch history:** если в branch есть merge commits — `git reset --soft` сохранит их. Если нужно линеаризовать — отдельная задача (rebase), не входит в `/squash`.
...
- [`./workflow.md`](workflow.md) — orchestrator (рекомендует `/squash` в финале)
- [`./evidence.md`](evidence.md) — DONE секция тоже ссылается на `/squash`
- [`./continue.md`](continue.md) — resume если прервался до evidence
