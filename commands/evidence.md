...
1. **Schema validation:** `node .gigacode/scripts/_validator.js validate .gigacode/schemas/evidence-bundle.schema.json <bundle-path>` → exit 0.
2. **AC coverage:** all `acceptance_criteria_status[].status == "passed"`.
3. **Record the transition (ОБЯЗАТЕЛЬНО).** Запиши phase_transition в audit-trail — `/squash` пропустит только задачу с фазой `evidence`/`done`:

   ```bash
   bash .gigacode/scripts/emit-phase-event.sh <task-id> review evidence <pass|fail> <iter> "<schema valid, AC N/N | что не так>"
   ```
...
- `header`: "Done→?"
- `options`:
  - `label`: "Squash & prepare PR (Recommended)" · `description`: "Запустить /squash для aggregated commit + push."
...
- `header`: "Fix?"
- `options`:
  - `label`: "Retry /evidence (Recommended)" · `description`: "Fix issues, iter counter +=1."
  - `label`: "Back to /implement" · `description`: "Issues указывают на реальные code-проблемы, не на bundle."
...
✗ Evidence phase: AC NOT passed
   ...
   Recovery rule (loop-recovery.md): откат на /implement (не Done — задача незакончена).
...
- `header`: "AC fail"
- `options`:
  - `label`: "Back to /implement (Recommended)" · `description`: "Fix failed AC; iter counter в /implement продолжает."
