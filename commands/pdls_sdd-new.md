...
2. **Worktree isolation.** Задача, заведённая через `/pdls:sdd-new`, выполняется в изолированном `git worktree` — весь downstream loop (plan → test → implement) не трогает основной checkout. После approve, до перехода к фазам, спроси через **`AskUserQuestion`**:
   ...
   Обработка ответа:
   - **Create worktree** → запусти `bash .gigacode/scripts/agentic-worktree.sh start <slug>` **сейчас** (worktree должен существовать до фазы /pdls:test, §2.5). Активируй skill `agentic-worktree` для lifecycle. Сообщи path + branch и далее веди весь loop внутри этого worktree.
