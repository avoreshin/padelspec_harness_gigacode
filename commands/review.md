...
2. **Вызови команду `/java-linter-review <subpath>`** (skill `java-linter-review`) и пройди её процесс целиком, включая гейт сборки SpotBugs.
   ...
   - `/java-linter-review` не является фазой loop'а — её phase-gate transition не выполняется, управление возвращается в `/review`.
...
```bash
bash .gigacode/scripts/emit-phase-event.sh <task-id> implement review <pass|fail> 1 "verdict: <APPROVE|REJECT>, critical=<n>, major=<n>"
```
(`pass` — только при APPROVE; REJECT любого уровня — `fail`.)
...
- `header`: "Review→?"
- `options`:
  - `label`: "Continue → /evidence (Recommended)" · `description`: "Собрать финальный evidence bundle."
...
Recovery rule (loop-recovery.md): откат на /implement для fix'а.
...
- `header`: "Reject→?"
- `options`:
  - `label`: "Back to /implement (Recommended)" · `description`: "Fix major findings в коде, iteration counter продолжает."
  - `label`: "Revise /plan" · `description`: "Findings указывают на gap в плане, не в коде."
...
⚠ Architectural rollback требуется (§9 GIGACODE.md, human required):
   Recovery rule (loop-recovery.md): откат на /sdd-new для SDD revision.
...
- `header`: "Critical!"
- `options`:
  - `label`: "Rollback → /sdd-new (Recommended)" · `description`: "Revise SDD: новый AC / INV / split task."
...
**Не запускай `/evidence` / `/implement` / `/sdd-new` автоматически.** Жди выбора пользователя.
