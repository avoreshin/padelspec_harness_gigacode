...
2. Субагент сам:
   - запустит `.gigacode/scripts/java-lint.sh` (детект JDK 17+, discovery `src/main`/`src/test`, запуск вендоренных линтеров → SARIF/XML);
   ...
   - вырежет сниппеты кода и **перезапишет тот же `report_file`** обогащённым отчётом **строго по образцу** `.gigacode/templates/java-lint-report.md` (группировка по файлам → Топ-5 категорий → Детальные заметки → Итого);
