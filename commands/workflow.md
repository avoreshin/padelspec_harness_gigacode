...
1. **Получи slug** из `{{args}}`. Если пустой — попроси у пользователя или предложи `/sdd-new <slug>` сначала.
...
2. **Найди SDD** в `docs/sdd/` по slug. Проверь:
   ...
   Если что-то не так → STOP, предложи `/sdd-new <slug>` для revision.
...
3. **Определи mode по risk_class:**
   ...
   | R3+ | **manual** | Confirm + warning «high-risk», предложить independent reviewer перед /test |
...
4. **Печать execution plan** перед стартом:
   ...
   - `header`: "Start?"
   - `options`:
     - `label`: "Start → /plan (Recommended)" · `description`: "Погнали по фазам plan → test → implement → review → evidence."
     - `label`: "Revise SDD first" · `description`: "Нужны правки в SDD перед стартом — возврат к /sdd-new."
...
5. **Последовательно запускай фазы:**
   ...
   - **/plan <slug>** → wait for phase-transition output
   - **status=pass** + auto-mode → auto-continue на /test
   - **status=pass** + manual-mode → ask «Готов к /test?»
...
6. **На фазе Implement (iteration loop):**
   ...
   - status=escalate → STOP, mandatory rollback на /sdd-new

7. **На фазе Review:**

   - verdict APPROVE → next phase (evidence)
   - REJECT major → recovery /implement (auto-mode: ask «продолжаем implement?»; manual: standard)
   - REJECT critical → STOP, architectural rollback /sdd-new (всегда manual confirm, **human required**)

8. **На фазе Evidence:**

   - bundle valid + AC all passed → DONE
   - bundle invalid → self-loop fix (max 3 iterations)
   - AC failed → rollback /implement
...
9. **При DONE:**
   ...
   - `header`: "Done→?"
   - `options`:
     - `label`: "Continue → /squash (Recommended)" · `description`: "Aggregated commit + подготовка PR."
...
- ❌ Если SDD не approved → STOP, не запускай /plan.
...
- [`./continue.md`](continue.md) — resume если прервался
- [`./squash.md`](squash.md) — финал перед PR
- [`./sdd-new.md`](sdd-new.md), [`./plan.md`](plan.md), [`./test.md`](test.md), [`./implement.md`](implement.md), [`./review.md`](review.md), [`./evidence.md`](evidence.md) — фазы loop
