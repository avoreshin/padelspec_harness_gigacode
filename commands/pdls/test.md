...
Запусти TDD-фазу для задачи: {{args}} (slug SDD; опц. `--tc=TAF-T1,TAF-T2` — брать acceptance criteria из тест-кейсов Zephyr вместо SDD).
...
| `{{args}}` с `--tc=<ключи>` | тест-кейсы Zephyr — `zephyr_get_test_case()` по каждому ключу | шаг 1 заменяется на шаг 1-TC |

Флаг опционален; без него поведение команды не меняется.

**Шаг 1-TC (вместо шага 1, если задан `--tc`).** Прочитать каждый TC через `zephyr_get_test_case()`, взять objective + Test Script как acceptance criteria, а `precondition` — как setup теста. Проверки: MCP доступен (иначе стоп с указанием, что нужно), каждый ключ разрешается в существующий TC (неразрешённый — стоп со списком), TC не пустой. SDD при этом не требуется; если он есть, используется как дополнительный контекст, но источником AC остаются TC.
