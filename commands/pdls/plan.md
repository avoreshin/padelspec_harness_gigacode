...
```bash
bash ${extensionPath}/scripts/emit-phase-event.sh <task-id> none plan pass 1 "plan ready: <N> files, <M> steps"
```
...
⚠ R<n> требует worktree (.worktrees/<task-id>/).
   Запусти `bash ${extensionPath}/scripts/agentic-worktree.sh start <task-id>` перед /pdls:test.
```
