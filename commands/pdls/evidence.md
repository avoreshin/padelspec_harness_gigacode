...
Шаги:
1. Прочитай schema: `${extensionPath}/schemas/evidence-bundle.schema.json`
2. Собери черновик детерминированно — `bash ${extensionPath}/scripts/collect-evidence.sh` (пишет `.gigacode/evidence-bundle.draft.json`): diff, изменённые файлы, git-контекст и risk class он снимает сам, из репозитория и `.gigacode/.cost/current-risk-class`. Дальше дополняются только те поля, которые из репозитория не выводятся — прогоны, assumptions, риски, статус AC. Собирать вручную то, что скрипт снимает точно, значит расходиться с ним в мелочах.
3. Дополни черновик:
...
4. Запиши результат в `.gigacode/evidence-bundle.json` строго по schema — это единственный путь, по которому bundle ищет Stop-хук (переопределяется переменной `EVIDENCE_BUNDLE_PATH`).

5. **Stop hook** (`evidence-bundle-enforcer.sh`) проверит наличие и валидность файла перед завершением сессии. Если bundle невалидный — задача останется in_progress.

6. Покажи краткую сводку пользователю:
   ```
