...
1. Выбери шаблон под тип задачи:
   - **frontend / UI** (компонент, экран, форма, Angular/React/Vue, вёрстка, UX) → `${extensionPath}/templates/SDD-frontend-template.md` (богатый UI/UX-блок: состояния, a11y WCAG, адаптивность, токены, темизация, component-контракт).
   - **всё остальное** (бэкенд, скрипты, инфра, доки) → `${extensionPath}/templates/SDD-template.md`.
