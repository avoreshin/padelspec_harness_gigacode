...
1. **Identify SDD.** Если `{{args}}` пустой — попроси slug SDD. Найди файл в `docs/sdd/`. Проверь:
   - status = `approved` (если `draft` → стоп, попроси approve через `/sdd-new` или явный re-approve)
   - acceptance_criteria count ≥ 1
   - AC написаны в Given-When-Then формате
   - Если что-то не так — STOP и предложи `/sdd-new <slug>` для revision.

2. **Delegate to `test` subagent.** Используй Agent tool с `subagent_type: "test"`. Передай:
   - Полный SDD content
   - Implementation plan (если есть в diff'е от `/plan`)
...
4. **Record the transition (ОБЯЗАТЕЛЬНО).** Запиши phase_transition в audit-trail:
   ...
   (`<status>` — из structured output шага 3; при self-loop test→test используй `from=test` и iteration+1.) Без этой записи `/continue` и `/squash` не увидят фазу.
...
6. **Next step (status=pass):**
   ...
   - `header`: "Test→?"
   - `options`:
     - `label`: "Continue → /implement (Recommended)" · `description`: "Тесты валидны и failing, переходим к минимальной реализации."
   ...
   **Fallback** (vendor без picker'а): напечатай numbered options и жди INPUT.
   ...
   В auto-mode (R0/R1 через `/workflow` при status=pass) переход к `/implement` автоматический.

   **Не запускай `/implement` автоматически в manual-mode.** Жди выбора.
...
7. **Recovery (status≠pass):**
   ...
   - `options`:
     ...
     - `label`: "Retry /test" · `description`: "Дать тестовому агенту ещё одну попытку."
