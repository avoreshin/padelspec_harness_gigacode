...
2. **Прочитай workflow state** через helper:
   ```bash
   bash .gigacode/scripts/workflow-state.sh <task-id> --text
   ```
...
3. **Обработай результат:**
   ...
   **a) Task не найден в audit** (exit 1):
   ...
   ⚠ Задача <task-id> не зафиксирована в audit-trail.
      Возможные причины:
      - SDD ещё не создан → запусти /sdd-new <slug>
   ...
   - `header`: "No task"
   - `options`:
     - `label`: "Create SDD → /sdd-new (Recommended)" · `description`: "Задача ещё не заведена — начинаем со спеки."
   ...
   **c) Task найден, status=iterate** (продолжаем итерацию):
   ...
   Recommended: /implement <task-id> (iteration <n+1>)
   ...
   - `options`:
     ...
     - `label`: "Revise SDD" · `description`: "Откат на /sdd-new — вероятен gap в acceptance criteria."
   ...
   **e) Task complete (current_phase=done):**
   ...
   ✓ Workflow для <task-id> завершён.
      ...
      Если ещё не открыт PR — рекомендую /squash <task-id> + git push.
   ```

4. **НЕ запускай команду автоматически** — `/continue` это manual-mode resume. Жди явного `yes`.
...
- [`./workflow.md`](workflow.md) — full orchestration (auto-mode для R0/R1)
- [`./squash.md`](squash.md) — финальный squash перед PR
- [`.gigacode/scripts/workflow-state.sh`](../../.gigacode/scripts/workflow-state.sh) — workflow state helper
