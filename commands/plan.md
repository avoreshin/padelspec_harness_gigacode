...
2. Если SDD не найден или имеет статус ≠ `approved` — остановись и потребуй от пользователя создать/утвердить SDD (`/sdd-new`)
...
**Запиши transition в audit-trail (ОБЯЗАТЕЛЬНО)** — без этого `/continue` и downstream-фазы не увидят, что plan завершён:
```bash
...
⚠ R<n> требует worktree (.worktrees/<task-id>/).
   Запусти `bash .gigacode/scripts/agentic-worktree.sh start <task-id>` перед /test.
```
...
- `header`: "Plan→?"
- `options`:
  - `label`: "Continue → /test (Recommended)" · `description`: "План валиден, переходим к написанию failing tests."
  - `label`: "Iterate plan" · `description`: "Доработать план; iteration counter += 1."
  - `label`: "Revise SDD" · `description`: "Plan выявил gap в SDD — возвращаемся через /sdd-new."
  - `label`: "Stop & save" · `description`: "Сохранить state в audit-trail, выйти. /continue вернёт."

В **auto-mode** (R0/R1 через `/workflow` при `status=pass`) picker пропускается, переход к `/test` автоматический. Failure / iterate всегда показывает picker (INV-3).
...
Plan готов. Что дальше?
  1) Continue → /test (Recommended)
  2) Iterate plan
  3) Revise SDD
  4) Stop & save
```

**Не запускай `/test` автоматически.** Жди выбора пользователя.
