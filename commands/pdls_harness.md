---
description: "Control panel: doctor · schemas · hooks list/enable/disable — диагностика и вкл/выкл хуков"
---
...
```
/pdls:harness — control panel

  doctor [--install]     проверка пакетов (git/node/npm, ajv…) + компиляция схем
  schemas [all]          валидация JSON-схем
  hooks list             показать хуки (активные + отключённые)
  hooks disable <имя>    отключить хук (обратимо)
...
## Если `{{args}}` НЕ пусто — разбери и выполни подкоманду (через `.gigacode/scripts/harness.sh`):

- `doctor` → `bash .gigacode/scripts/harness.sh doctor` — бинарники (git/node/npm), опциональные (perl/pip3/pre-commit/rsync), node-пакеты (**ajv**, ajv-formats, js-yaml) + компиляция схем. Exit 0 = всё ок.
- `doctor --install` → `bash .gigacode/scripts/harness.sh doctor --install` — то же + one-time установка node-пакетов.
- `schemas` / `schemas all` → `bash .gigacode/scripts/harness.sh schemas all` — валидация JSON-схем.
- `hooks list` → `bash .gigacode/scripts/harness.sh hooks list` — активные + отключённые хуки.
