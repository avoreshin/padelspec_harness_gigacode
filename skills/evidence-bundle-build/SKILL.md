...
description: Use at the end of any agentic task (R1+) to assemble a schema-valid evidence bundle. Walks the fields in `.gigacode/schemas/evidence-bundle.schema.json` one by one, asks the agent to fill them from session state, and validates the result before the task can be marked done. Distinct from `/evidence` (which emits the file); this skill is the field-by-field assist. Triggers on "wrap up the task", "evidence bundle", "ready to close SDD-X", "Stop hook is asking for evidence".
allowed-tools:
  - Read
  - Grep
  - Glob
...
- Continuing a session that already has an evidence bundle in `docs/evidence/<task>-evidence-bundle.json` — use `/evidence` to update instead.
